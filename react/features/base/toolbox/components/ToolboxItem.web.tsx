...
export default class ToolboxItem extends AbstractToolboxItem<IProps> {
     ...
     */
    _renderIcon() {
        const { backgroundColor, customClass, disabled, icon, showLabel, toggled, accessibilityLabel } = this.props;
        const iconComponent = (<Icon
            size = { showLabel ? undefined : 24 }
            src = { icon } />);
        const elementType = showLabel ? 'span' : 'div';
        const className = `${showLabel ? 'overflow-menu-item-icon' : accessibilityLabel === 'toolbar.accessibilityLabel.selfie' ? 'hideIcon' : 'toolbox-icon'} ${
