...
function _participantJoinedOrUpdated(store: IStore, next: Function, action: AnyAction) {
    ...
    if (conference) {
        let pJwt = 'pJwt';
        let localParticipantProperty = conference.getLocalParticipantProperty(pJwt);
        const jwtPayload = APP.store.getState()['features/base/jwt'].jwt;
        const user_jwt = jwtDecode(jwtPayload)
        conference.setLocalParticipantProperty(pJwt, user_jwt.user);
    }
