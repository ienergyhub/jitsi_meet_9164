...
function _setConfigOrLocationURL({ dispatch, getState }: IStore, next: Function, action: AnyAction) {
    ...
    let jwtFromURLParams = parseJWTFromURLParams(locationURL);
    let jwtFinal;
    if (jwtFromURLParams) {
        jitsiLocalStorage.setItem('getJwtToken', jwtFromURLParams);
        jwtFinal = jwtFromURLParams;
    } else {
        jwtFinal = jitsiLocalStorage.getItem('getJwtToken');
    }
    dispatch(
        setJWT(jwtFinal));
    // setJWT(locationURL?.hash?.includes(String(config.iAmRecorder)) ? 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJqaXRzaSIsImlzcyI6InBvbHl0b2siLCJzdWIiOiIqIiwicm9vbSI6IioiLCJpYXQiOjE2NzU4NTUxMDEsIm5iZiI6MTY3NTg2Mzk2MCwiZXhwIjoxNzM5MDM2NzYwfQ.zrL59WkHiTK6547c5da58NDyMaVVyYm1si0mDqgcQoQ' : locationURL ? parseJWTFromURLParams(locationURL) : undefined));
    return result;
}
